...
-- Trigger για τον έλεγχο κατά την ΕΙΣΑΓΩΓΗ (INSERT)
...
-- Trigger για τον έλεγχο κατά την ΕΝΗΜΕΡΩΣΗ (UPDATE) - Αποφυγή Κυκλικής Εποπτείας
CREATE TRIGGER trg_iatroi_supervision_update
BEFORE UPDATE ON Iatroi
FOR EACH ROW
BEGIN
    IF NEW.amka_epopti IS NOT NULL THEN
        -- Έλεγχος κυκλικής αλυσίδας (Α επόπτης του Β, Β επόπτης του Α)
